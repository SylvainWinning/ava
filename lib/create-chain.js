...
function callWithFlag(previous, flag, args) {
	callChain(previous, {[flag]: true}, args);
}

function callChain(previous, flags, args) {
...
function createCombinedHookChain(first, second, name) {
	const fn = (...args) => {
		callChain(first, {}, args);
		callChain(second, {}, args);
	};
...
function addBeforeAfterChain(beforeHook, afterHook) {
	beforeHook.and = {
		after: createCombinedHookChain(beforeHook, afterHook.always, `${beforeHook.name}.and.after`),
	};
}

function addBeforeEachAfterChain(beforeEachHook, afterHook, afterEachHook) {
	beforeEachHook.and = {
		after: createCombinedHookChain(beforeEachHook, afterHook.always, `${beforeEachHook.name}.and.after`),
		afterEach: createCombinedHookChain(beforeEachHook, afterEachHook.always, `${beforeEachHook.name}.and.afterEach`),
	};
}
...
export default function createChain(fn, defaults, meta) {
	...
	addBeforeAfterChain(root.before, root.after);
	addBeforeEachAfterChain(root.beforeEach, root.after, root.afterEach);
	...
	addBeforeAfterChain(root.serial.before, root.serial.after);
	addBeforeEachAfterChain(root.serial.beforeEach, root.serial.after, root.serial.afterEach);
